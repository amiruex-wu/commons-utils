...
public class NumberUtils {
    ...
    // -----------------------------------------------------------------------
    public static int defaultIfNull(final Integer integer, final int defaultValue) {
        return ObjectUtils.nonNull(integer) ? integer : defaultValue;
    }
    ...
    public static Optional<Integer> max(final Integer a, final Integer b) {
        if (ObjectUtils.allNull(a, b)) {
            return Optional.empty();
        }
        final int max = Math.max(defaultIfNull(a, 0), defaultIfNull(b, 0));
        return Optional.ofNullable(max == 0 ? null : max);
    }

    public static Optional<Integer> max(final Integer... array) {
        try {
            validateArray(array);
            Integer max = array[0];
            for (int j = 1; j < array.length; j++) {
                final int a = defaultIfNull(array[j], 0);
                final int b = defaultIfNull(max, 0);
                if (a > b) {
                    max = array[j];
                }
            }
     ...
     * 判断目标是否是基数的整数倍
     *
     * @param baseInteger 基数
     * @param target      目标数据
     * @return 返回是否
     */
    public static boolean isIntegerMultiple(Integer baseInteger, Integer target) {
        if (ObjectUtils.anyNull(baseInteger, target)) {
            return false;
        }
        if (target.compareTo(baseInteger) < 0) {
            return false;
        }
        final int remaining = target % baseInteger;
        return remaining == 0;
    }
}
